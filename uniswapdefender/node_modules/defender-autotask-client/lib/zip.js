...
async function zipFolder(folderPath) {
    if (!fs_2.existsSync(path_1.join(folderPath, 'index.js')))
        throw new Error(`Expected index.js entrypoint in folder`);
    const files = await util_1.promisify(glob_1.default)('**', { cwd: folderPath, nodir: true });
    const zip = new jszip_1.default();
    for (const path of files) {
        const content = await util_1.promisify(fs_1.readFile)(path_1.join(folderPath, path));
